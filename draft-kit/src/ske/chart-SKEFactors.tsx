...
import { SKE_FACTORS, CQI_COLOR, HORIZON_LABEL, type Horizon, type FactorRow } from './data-ske';
import { shapArrowPts } from '../draft/shared';

type UnitMode = 'mj' | 'mwon';
type SortMode = 'cumulative' | 'magnitude';

const HORIZONS: Horizon[] = ['D-1', 'W-1', 'M-1'];

function fmtImpact(v: number, unit: UnitMode) {
  const sign = v >= 0 ? '+' : '';
  if (unit === 'mj') return `${sign}${v.toFixed(1)} MMJ`;
  const bwon = v / 1000;
  if (Math.abs(bwon) >= 0.1) return `${sign}${bwon.toFixed(2)} B원`;
  return `${sign}${v.toFixed(0)} M원`;
}
...
export function SKEFactorsChart() {
  ...
  const impactKey = unit === 'mj' ? 'impact_mj' : 'impact_mwon';

  const rows = useMemo(() => {
    const base = SKE_FACTORS.filter(r => r.horizon === horizon);
    if (sortMode === 'magnitude')
      return [...base].sort((a, b) => Math.abs(b[impactKey]) - Math.abs(a[impactKey]));
    // cumulative: 양수 큰 순 → 음수 큰 순
    ...
    return [...pos, ...neg];
  }, [horizon, unit, sortMode, impactKey]);
  ...
  return (
    <div className="draft-chart-card" style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      <div className="draft-ekpi-card-title">
        <span>운전요인 변동 기여도 (Waterfall)</span>
        <div className="draft-card-actions">
          ...
          <div style={{ display: 'flex', border: '1px solid #334155', borderRadius: 6, overflow: 'hidden' }}>
            {(['magnitude', 'cumulative'] as SortMode[]).map(s => (
              <button key={s}
                ...
                style={{ borderRadius: 0, borderRight: s === 'magnitude' ? '1px solid #334155' : 'none', fontSize: 10 }}
          ...
          {/* 단위 */}
          <div style={{ display: 'flex', border: '1px solid #334155', borderRadius: 6, overflow: 'hidden' }}>
            {(['mj', 'mwon'] as UnitMode[]).map(u => (
              <button key={u}
                className={`draft-chip-btn${unit === u ? ' draft-chip-btn--active' : ''}`}
                style={{ borderRadius: 0, borderRight: u === 'mj' ? '1px solid #334155' : 'none' }}
                onClick={() => setUnit(u)}>
                {u === 'mj' ? 'MJ' : '비용(원)'}
              </button>
            ))}
      ...
      <div style={{ display: 'flex', gap: 2, padding: '2px 12px 6px' }}>
        ...
        <span style={{ marginLeft: 8, fontSize: 11, color: '#475569', alignSelf: 'center' }}>
          합계 <strong style={{ color: total >= 0 ? '#86efac' : '#fca5a5' }}>{fmtImpact(total, unit)}</strong>
        </span>
      </div>
      ...
      <div ref={wrapRef} className="draft-chart-wrap" style={{ flex: 1, minHeight: 0 }}>
        <svg ref={svgRef} className="draft-chart-svg" width={size.w} height={size.h}
          ...
          <text x={xSc(0)} y={PAD_T - 8} textAnchor="middle" fontSize={9} fill="#64748b">0</text>
          <text x={xSc(total)} y={PAD_T - 8} textAnchor="middle" fontSize={9}
            fill={total >= 0 ? '#86efac' : '#fca5a5'}>
            {fmtImpact(total, unit)}
          </text>
          ...
          {rows.map((row, i) => {
            ...
            const inside = bw > 52;
            const valStr = fmtImpact(impact, unit);
            ...
            return (
              <g key={row.group} style={{ cursor: 'pointer' }}
                ...
                onMouseLeave={() => setTooltip(null)}>
                <polygon points={shapArrowPts(x1, x2, cy, bh)} fill={row.color} opacity={0.87} />
                <text x={LABEL_W - 8} y={cy + 4} textAnchor="end" fontSize={11}
                  fill="#e2e8f0" fontWeight="600">
                  {row.group}
                </text>
                <text x={lx} y={cy + 4} textAnchor={la} fontSize={10}
                  fill={inside ? '#fff' : (impact >= 0 ? '#86efac' : '#fca5a5')} fontWeight="600">
                  {valStr}
                </text>
              </g>
            );
          ...
          {tooltip && (() => {
            ...
            return (
              <g>
                ...
                <text x={tipX + 22} y={tipY + 18} fontSize={11} fill="#e2e8f0" fontWeight="700">
                  {row.group}
                </text>
                <text x={tipX + 8} y={tipY + 34} fontSize={10}
                  fill={impact >= 0 ? '#86efac' : '#fca5a5'} fontWeight="600">
                  {fmtImpact(impact, unit)}
                </text>
                {rawStr && (
                  <text x={tipX + 8} y={tipY + 50} fontSize={9} fill="#64748b">{rawStr}</text>
                )}
                {/* CQI */}
                <rect x={tipX + 8} y={tipY + tipH - 18} width={56} height={12} rx={3}
                  fill={CQI_COLOR[row.cqi_level]} opacity={0.18} />
                <text x={tipX + 36} y={tipY + tipH - 8} textAnchor="middle" fontSize={9}
      ...
      <div style={{ padding: '2px 12px 8px', fontSize: 10, color: '#475569' }}>
        단위: {unit === 'mj' ? 'M MJ' : 'M원'} · {HORIZON_LABEL[horizon]} · 정렬: {sortMode === 'magnitude' ? '절대값 크기순' : '누적 흐름순'}
      </div>
    </div>
  );
}
