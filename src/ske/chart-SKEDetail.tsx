...
import {
  getSKEDetail, GROUP_COLORS, ENERGY_COLOR, DATE_RANGE_LABEL,
  FEATURE_DESC,
  type EnergyType, type DetailRow, type DateRange,
} from './data-ske';
...
interface SKEDetailChartProps {
  selectedGroup?: string | null;
  activeEnergy?: EnergyType;
  dateRange?: DateRange;
}

export function SKEDetailChart({ selectedGroup: groupProp, activeEnergy, dateRange = '30d' }: SKEDetailChartProps = {}) {
  const wrapRef = useRef<HTMLDivElement>(null);
  const size = useContainerSize(wrapRef);
  const svgRef = useRef<SVGSVGElement>(null);

  const [internalEnergy] = useState<EnergyType>('FG');
  const energy = activeEnergy ?? internalEnergy;
  const unit = 'mj' as const;
  const [sortMode, setSortMode] = useState<'magnitude' | 'group' | 'cumulative'>('magnitude');
  const [tooltip, setTooltip] = useState<{ row: DetailRow; svgX: number; svgY: number } | null>(null);
  const selectedGroup = groupProp ?? null;

  const rows: DetailRow[] = useMemo(() => {
    let data = getSKEDetail(dateRange).filter(r =>
      r.horizon === 'D-1' &&
      r.energy === energy &&
    ...
    if (sortMode === 'magnitude')
      data = [...data].sort((a, b) => Math.abs(getVal(b)) - Math.abs(getVal(a)));
    else if (sortMode === 'cumulative') {
      const pos = data.filter(r => getVal(r) >= 0).sort((a, b) => getVal(b) - getVal(a));
      const neg = data.filter(r => getVal(r) < 0).sort((a, b) => getVal(a) - getVal(b));
      data = [...pos, ...neg];
    } else
      data = [...data].sort((a, b) => a.group.localeCompare(b.group) || a.feature.localeCompare(b.feature));
    return data;
  }, [energy, sortMode, selectedGroup, dateRange]);
    ...
    <div className="draft-chart-card" style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '3px 8px', flexShrink: 0, flexWrap: 'wrap' }}>
        ...
        <div style={{ display: 'flex', border: '1px solid #334155', borderRadius: 6, overflow: 'hidden' }}>
          {([['magnitude', '크기순'], ['cumulative', '누적순'], ['group', '그룹순']] as const).map(([s, label]) => (
            <button key={s}
              className={`draft-chip-btn${sortMode === s ? ' draft-chip-btn--active' : ''}`}
              style={{ borderRadius: 0, border: 'none', borderRight: s !== 'group' ? '1px solid #334155' : 'none', fontSize: 10 }}
      ...
      <div ref={wrapRef} className="draft-chart-wrap" style={{ flex: 1, minHeight: 0 }}>
        <svg ref={svgRef} className="draft-chart-svg" width={size.w} height={size.h}
          ...
          <text x={xSc(total)} y={PAD_T - 6} textAnchor="middle" fontSize={11} fill="#7dd3fc">
            ...
            return (
              <g key={`${row.horizon}-${row.energy}-${row.feature}`} style={{ cursor: 'pointer' }}
                ...
                <text x={LABEL_W - 6} y={cy + 4} textAnchor="end" fontSize={11}>
                  ...
                  <tspan fill="#334155" fontSize={10}> {row.group}</tspan>
                </text>
          ...
          {tooltip && (() => {
            ...
            return (
              <g>
                ...
                {/* 설명 */}
                {desc && (
                  <text x={tipX + 8} y={tipY + 30} fontSize={11} fill="#64748b">{desc}</text>
                )}
                {/* 그룹 */}
                <circle cx={tipX + 12} cy={tipY + (desc ? 44 : 30)} r={4} fill={gColor} />
                <text x={tipX + 22} y={tipY + (desc ? 48 : 34)} fontSize={11} fill="#94a3b8">
                  {row.group}
                </text>
                {/* SHAP 값 */}
                ...
                <text x={tipX + 8} y={tipY + tipH - 8} fontSize={11} fill="#475569">
      ...
      <div style={{ padding: '2px 12px 8px', fontSize: 12, color: '#475569' }}>
        {energy} ({ENERGY_UNIT[energy]}) · M MJ · {sortMode === 'magnitude' ? '크기순' : sortMode === 'cumulative' ? '누적순' : '그룹순'} · {rows.length}개 피처
      </div>
    </div>
  );
}
